...
const createUser = async (req, res) => {
  try {
    ...
    // :white_check_mark: Check if role exists
    const roleExists = await Roles.findById(role);
    if (!roleExists) {
      return res.status(400).json({ status: 400, message: "Invalid role ID" });
    }
    // :white_check_mark: Generate unique userId like "usr-0001"
    ...
    // :white_check_mark: Create new user with generated ID
    const user = new User({
      userId,
      name,
      email,
      password, // :warning: Consider hashing later with bcrypt
      role,
      status,
    });
    await user.save();
...
const updateUser = async (req, res) => {
  try {
    ...
    if (!role) missingFields.push({ name: "role", message: "Role is required" });
    if (status === undefined) missingFields.push({ name: "status", message: "Status is required" });
    // :x: password ko required mat banao
    // :white_check_mark: sirf tab update karo jab user ne diya ho
    if (missingFields.length > 0) {
      return res.status(400).json({
        status: 400,
        message: "Validation failed",
    ...
    // :white_check_mark: check valid role id
    if (role) {
      const roleExists = await Roles.findById(role);
      if (!roleExists) {
        return res.status(400).json({ status: 400, message: "Invalid role ID" });
    ...
    // :white_check_mark: update object dynamically build karo
